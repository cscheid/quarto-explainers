    ...
    return function(sel) {
        var gs = sel
            .enterMany(ticks.list)
                .append("g");
        gs.append("line")
            .attr("x1", function(d) { return ticks.scale(d.v || d); })
            .attr("x2", function(d) { return ticks.scale(d.v || d); })
            .attr("y1", -5).attr("y2", 5)
            .attr("class", "test-line");
        gs.append("text")
            .attr("x", function(d) { return ticks.scale(d.v || d); })
            .attr("y", -15)
        ...
        .text(function(d) {
          ...
          return ticks.fmt(Math.round(d * 100000) / 100000);
        });
    };
}
...
var postTestHandle = svg.append("circle")
        ...
        .call(d3.drag().on("drag", function(e) {
            d3.select(this).attr("cx", e.x);
            lrHandle.attr("cx", 0.5 * (e.x + Number(preTestHandle.attr("cx"))));
            updateLine();
        }));
